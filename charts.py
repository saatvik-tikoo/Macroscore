...
def clean_data_local():
    ...
    with open("Psychology/rpp_data_codebook.csv") as input_file:
        reader = macroscope.csv.reader(input_file)
        # skip the header
        next(reader)
        for line in reader:
            if '(R)' not in line[0] and line[4] in data_types and line[0] not in remove_columns:
                req_columns.append(line[0])
    data_set = macroscope.pandas.read_csv('Psychology/rpp_data_updates.csv', encoding='ansi', usecols=req_columns)

    # Add output field and dropping the Replication fields
    data_set['result_label'] = data_set.apply(lambda row: macroscope.label_addition(row), axis=1)
    data_set = data_set.drop(['P-value (R)', 'Direction (R)'], axis=1)

    # Clean fields
    data_set = data_set.replace(to_replace='X', value=macroscope.np.nan)
    data_set = data_set.replace(to_replace=macroscope.np.nan, value=0)
    data_set = macroscope.clean_pvalue(data_set, 'Reported P-value (O)')
    data_set = macroscope.clean_range_values(data_set, 'Pages (O)')
    data_set = macroscope.clean_float_result(data_set, 'Surprising result (O)')
    data_set = macroscope.clean_float_result(data_set, 'Exciting result (O)')
